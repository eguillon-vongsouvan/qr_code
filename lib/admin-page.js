...
function renderAdminPage({ error = '', message = '' }, userId = null) {
        ...
        <div style="margin-bottom:1.25rem;">
          ${(function(){
            ...
            return '<div style="display:flex;flex-direction:column;gap:0.4rem;">' + users.map(u => 
              `<div style="display:flex;justify-content:space-between;align-items:center;padding:0.7rem 1rem;border:1px solid var(--border);border-radius:10px;background:var(--bg-elevated);transition:all 0.15s;" id="user-row-${escapeHtml(u.id)}">
                 <div>
                   <strong style="color:var(--text-main);font-size:0.92rem;">${escapeHtml(u.username)}</strong>
                 ...
                 ${u.username !== 'admini' ? `<button class="btn-delete-user" style="padding:0.3rem 0.7rem;font-size:0.78rem;font-weight:600;border-radius:6px;background:transparent;color:#f87171;border:1px solid rgba(239,68,68,0.2);cursor:pointer;transition:all 0.15s;font-family:inherit;" data-user-id="${escapeHtml(u.id)}">Supprimer</button>` : ''}
               </div>`
            ).join('') + '</div>';
          })()}
        </div>
